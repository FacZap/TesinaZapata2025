...
T_end(3) = Tcout_1_end;
T_end(4) = Tcout_2_end;
d_est_vector(1) = (Thout_1_end - Thout_1_ini)/delta; d_est_vector(2) = (Thout_2_end - Thout_2_ini)/delta;
d_est_vector(3) = (Tcout_1_end - Tcout_1_ini)/delta; d_est_vector(4) = (Tcout_2_end - Tcout_2_ini)/delta;
for i = 1:4
    d_est(i,d_selected) = d_est_vector(i)
end
t_vect=tout;
...
for k=selected_outputs
    D_modelo(k,d_selected)=tf(d_est(k,d_selected),[t_corte(k) 1])
end
for k=not_selected_outputs
    D_modelo(k,d_selected)=tf(0)
end
