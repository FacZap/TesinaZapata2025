...
for j=selected_outputs
    t_aux1=(t_vect(find(abs(y.data(:,j)-salida_pto_op(j))>abs(T_end(j)-salida_pto_op(j))*1.38,1,'last')) - t_step);
    t_aux2=(t_vect(find(abs(y.data(:,j)-salida_pto_op(j))<abs(T_end(j)-salida_pto_op(j))*0.62,1,'last')) - t_step);
    if isempty(t_aux1)
        t_aux1=0;
    end
    if isempty(t_aux2) 
