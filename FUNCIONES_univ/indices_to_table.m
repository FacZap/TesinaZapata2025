...
if (k_table==1)
    N = 12;
    X = nan(N, numel(colNames_X));
    Y = nan(N, numel(colNames_Y))
end

X(k_table,:) = [k_table, IAE(1), IAE(2), IAE(3), IAE(4), IAE_total];
Y(k_table,:) = [k_table, Eu(1), Eu(2), Eu(3), Eu(4), Eu_total, Eu_total_utilidades]
Z(k_table,:) = [k_table, T_est(1), T_est(2), T_est(3), T_est(4)];
